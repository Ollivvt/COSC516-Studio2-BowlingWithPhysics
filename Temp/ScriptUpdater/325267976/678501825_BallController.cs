...
public class BallController : MonoBehaviour
{
    ...
    public void SetInGutter(bool value)
    {
        ...
        if (value)
        {
            // Reduce lateral movement but allow smooth rolling
            Vector3 currentVelocity = ballRigidBody.linearVelocity;
            currentVelocity.x *= 0.05f; // Ensure minimal side movement
            currentVelocity.z *= 0.7f; // Reduce forward speed gradually
            ballRigidBody.linearVelocity = currentVelocity;

            // **Adjust launch indicator position to stay aligned**
            if (launchIndicator != null)
            {
                Vector3 launchIndicatorPosition = launchIndicator.position;
                launchIndicatorPosition.x = transform.position.x; // Keep it centered
                launchIndicator.position = Vector3.Lerp(launchIndicator.position, launchIndicatorPosition, 0.2f);
            }
        }
    }
}
